<x-app-layout>
    <x-slot name="header">
        ...
        <div class="row justify-content-center align-items-center text-center g-2 mt-4 px-5">
            <div class="col-md-10 col-lg-10 col-sm-12 mb-2">
                <input type="text" class="form-control fs-5" placeholder="Paste your link" name="" id="urlInput">
                <div id="validationServerUsernameFeedback" class="invalid-feedback">
                    Enter Valid Url
                </div>
            </div>
            <div class="col-sm-6 col-md-2 col-lg-2 mb-2">
                <button class="btn btn-danger text-light fs-5 px-3" onclick="sohwLinkPreview()" id="editButton">Edit
                    Preview</button>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <center>
                        <h2 class="display-5">{{ __('Welcome to ' . env('APP_NAME') . '!') }}</h2>
                        <p class="lead">{{ __('Create a new link above to get started') }}</p>
                    </center>
                </div>
            </div>
        </div>
    </div>
    <div style="min-height: 35vh;" class="pb-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="row justify-content-center align-items-center g-2">
                        <div class="col-md-2 col-lg-2 col-sm-12 text-c ms-auto mb-3">
                            <a class="btn btn-outline-dark"
                                href="@if (!$is_archive) {{ route('card.archive') }}@else {{ route('dashboard') }} @endif">
                                @if (!$is_archive)
                                    View archived
                                @else
                                    View Actives
...
$redirect_url = $card->redirect_url;
                            ...
                            {{-- <td>
                                <button
                                    onclick="sohwLink('{{ $card->id }}','{{ $card->redirect_url }}','{{ $card->title }}','{{ $card->description }}','{{ $card->domain }}')"
                                    class="text-success"><i class="fa-solid fa-pen-to-square"></i></button>
                                <form style="display: inline-block;" class="ms-3"
                                    action="{{ route('card.destroy', ['card' => $card->id]) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button type="sumbit" class="text-danger"><i
